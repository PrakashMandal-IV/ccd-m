...
exports.getMyNotifications = async (req, res) => {
    try {
        ...
        const list = await notificationModel.find(filter).populate('sentBy', 'name refId').exec()
        var response = []
        list.forEach(item => {
        ...
        return res.error("Error occurred while fetching notification", error.message);
    }
}
